...
void print_output_atomic(int num_threads, std::atomic<int>& primes, std::atomic<int>& nonprimes, double mean, std::vector<std::atomic<int>>& number_counts, std::chrono::high_resolution_clock::time_point t1, std::chrono::high_resolution_clock::time_point t2, bool only_exec_times, bool no_exec_times) {
    if ( !only_exec_times ) {
        std::cout << "Primes: " << primes << std::endl;
        std::cout << "Nonprimes: " << nonprimes << std::endl;
        std::cout << "Mean: " << std::fixed << std::setprecision(2) << mean << std::endl;
